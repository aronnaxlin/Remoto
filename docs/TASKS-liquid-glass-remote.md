...
- [ ] **底部浮动玻璃模式条**:遥控 / ⌨️键盘 / ▦更多 —— Tab 的视觉、Sheet 的行为(见 §七),承接 Task 4/5 的入口
...
- [ ] 入口:遥控主页底部**浮动玻璃模式条**的「⌨️ 键盘」项(见 §七;Tab 的视觉、Sheet 的行为,不做 TabView)
- [ ] 点击后玻璃输入条从底部升起盖住模式条,**方向键原地不动** —— 打字→选结果→再打字是高频交错,不允许页面跳转
- [ ] 发送走 `session.sendText()`;`.unsupported` 错误 → 提示「先在电视上打开一个搜索框」
- [ ] 这是对标 Apple TV Remote「键盘自动弹出」的核心差异化功能,交互要做顺:输入中实时回显、发送后清空并给成功反馈

### Task 5 — 二级面板(更多按键)
- [ ] 入口:模式条的「▦ 更多」项;半屏面板升起(`.presentationDetents`),主页在底下隐约可见:media 控制(播放/暂停/快进等 common 键)
- [ ] 数字键盘(0–9,折叠)
...
**单核心页 + 四个入口**:90% 时间停在遥控主页,其他页面都是路过。

```
启动 ──→ 已配对过? ──是──→ 遥控主页(自动重连上次电视)
              │
              否──→ 发现页 ──点电视──→ PSK配对页 ──→ 遥控主页
```

1. **发现页(只出现一次)**:深色背景,玻璃卡片列出电视(型号+在线状态),「手动输 IP」弱入口;权限被拒在此页引导去系统设置
2. **遥控主页(核心)**:顶部电视名(可切换多台)+状态;中央玻璃触控板(或 ClickWheel 皮肤);一行玻璃圆键(返回/主页/静音/电源);音量长条;**底部浮动玻璃模式条:遥控 / 键盘 / 更多**(见下)
3. **模式条的两个出口(Tab 的视觉,Sheet 的行为)**:
   - 点「键盘」→ 玻璃输入条从底部升起盖住模式条,**方向键原地不动**,打完即达电视输入框
   - 点「更多」→ 半屏面板升起(播放控制、折叠数字键盘、品牌专属键区、设备信息),主页在底下隐约可见
   - 设计理由:遥控是「眼在电视、手在手机」的盲操场景,键盘/更多键依附于当前控制会话,不是平级页面;不做 TabView,不做页面跳转,但保留 Tab 式的可见入口(解决上滑手势/小图标太隐蔽的问题)
4. **PSK 配对页(仅首次,模态)**:型号+步骤+输入框,成功即存 Keychain 永不复现

入口原则:**遥控主页是终点不是中转站** —— 打开 App 两秒内手指一定能碰到方向键。
