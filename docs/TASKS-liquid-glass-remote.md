...
2. **遥控主页(核心)**:顶部电视名(可切换多台)+状态;中央玻璃触控板(或 ClickWheel 皮肤);一行玻璃圆键(返回/主页/静音/电源);音量长条;底部抓手「上滑更多」
3. **二级面板(主页上滑半屏)**:播放控制、折叠数字键盘、品牌专属键区、「这台电视」信息
4. **远程键盘(主页固定入口)**:玻璃输入条从底部升起,打字发送直达电视输入框
5. **PSK 配对页(仅首次,模态)**:型号+步骤+输入框,成功即存 Keychain 永不复现
